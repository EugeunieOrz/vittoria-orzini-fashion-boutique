package utils.core.mongo

import utils.core.mongo.exceptions.MongoException
...
trait MongoModel {
  ...
  protected def find[T](
    ...
    queryModifier.map(f => f(coll.find(query).snapshot())).getOrElse(coll.find(query).snapshot())
  ...
  protected def onSuccess[T](result: Future[WriteResult], entity: T): Future[T] = result.recoverWith {
    case e =>
      Future.failed(new MongoException("Got exception from MongoDB", Some(e.getCause)))
  }.map { r =>
    WriteResult.lastError(r) match {
      case Some(e) =>
        throw new MongoException(e.message, Some(e))
      case _ =>
        entity
    }
  }
