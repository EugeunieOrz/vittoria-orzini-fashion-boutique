...
import { changeQtyInGuestShoppingBag } from 'modules/Product/ProductViewModule';
...
export function* editQtyInBagSaga(api: ShoppingAPI): Generator<*, *, *> {
  while (true) {
    const { payload } = yield take(changeQtyInGuestShoppingBag().type);
    try {
      const response = yield call([api, api.editProductQtyInGuestShoppingBag], payload);
