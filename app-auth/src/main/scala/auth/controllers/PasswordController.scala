...
import core.models.PasswordSurvey
...
class PasswordController @Inject() (
        ...
        password => userService.retrieve(authToken.userID).flatMap {
          case Some(user) if user.loginInfo.exists(_.providerID == CredentialsProvider.ID) =>
            ...
            authInfoRepository.update[PasswordInfo](loginInfo, passwordInfo).map { _ =>
              Ok(ApiResponse("auth.password.reset.successful", Messages("auth.password.reset"), authToken.userID.toString))
            }
  ...
  def submitPasswordSurvey: Action[AnyContent] =
    silhouette.UnsecuredAction.async { implicit request =>
      println("Data in request: " + request.body)
            ...
            userService.retrieve(value.userID).flatMap {
              case Some(user) if user.loginInfo.exists(_.providerID == CredentialsProvider.ID) =>
                userService.save(user.copy(passwordSurvey =
                  ...
                  Ok(ApiResponse("auth.password.survey.successful", Messages("auth.password.survey")))
                }
              case _ => Future.successful(
                BadRequest(ApiResponse("auth.password.survey.invalid", Messages("auth.invalid.survey")))
              )
            }
          case e: JsError => Future.successful(
            BadRequest(ApiResponse(
              "auth.password.survey.info.invalid",
              Messages("auth.invalid.survey"), JsError.toJson(e).toString()))
          )
        }
      }
      Future.successful(Ok(ApiResponse("auth.password.survey.successful", Messages("auth.password.survey.successful"))))
    }
